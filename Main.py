# ############################################################################################# #
# Modified and implemented Filtersim algorithm by Sebastian Avalos @ Universidad de Chile - 2013 #
# ############################################################################################# #
# Main file in which the complete filtersim process is done
...
print "This version include the local mean as a influence parameter."
...
for ind in xrange(nmgrid):
   xx = (2**(ind))*(ngsx-innerx)/2
   yy = (2**(ind))*(ngsy-innery)/2
   for enx in xrange(sgnumx+(2**(ind))*(-ngsx+innerx)):
      for eny in xrange(sgnumy+(2**(ind))*(-ngsy+innery)):
         if ((enx+xx)%2**(ind)==0) and ((eny+yy)%2**(ind)==0):
            GSval[ind][enx + (xx)][eny + (yy)] = 1
            #Listos[ind][enx + (xx)][eny + (yy)][enz + (zz)] = 1
...
Pattern = np.zeros((nmgrid,7,ncluster)) #estos seran los patrones de cada cluster
for ind in xrange(nmgrid):
...
for trab in xrange(repeticion):
               ...
               if cntd>=minimumdc and vuelta == 1:
                  for ind4 in xrange(ncluster):
                     ...
                     if cont[ind][ind4]>0:
                        ...
                        p1 = p1.sum()
                        #promActual = SimGrid[menx-xx:menx-xx+ngsx,meny-yy:meny-yy+ngsy,trab]*AvLoc[menx-xx:menx-xx+ngsx,meny-yy:meny-yy+ngsy,trab]
                        promActual=np.mean(FinalPattern[ind][ind4])
                        #print promActual, meanSG[menx][meny]
                        #print vuelta
                        p=p1*(1-w0)+w0*(abs(meanSG[menx][meny] - promActual))
                        #TODO usar slices y operaciones sobre matrices. usar np.where para procesar solo los que cumplen una condicion
                  ...
                  #print np.mean(FinalPattern[ind][ind4]), meanSG[menx][meny] 
                  #print vuelta                  
                  ...
                  #SimGrid[menx-xx3:menx-xx3+innerx,meny-yy3:meny-yy3+innery,trab] = depaso4[eleccion][xx2:(ngsx-xx2-1),yy2:(ngsy-yy2-1)]
                  ...
                  #AvLoc[menx-xx3:menx-xx3+innerx,meny-yy3:meny-yy3+innery,trab]=1
               ...
               if vuelta>1:
                  ...
                  if vacio>0:
                     for ind4 in xrange(ncluster):
                        p = 0.0
                        if cont[ind][ind4]>0:
                           #p = Pond[menx-xx:menx-xx+ngsx,meny-yy:meny-yy+ngsy,trab]*(abs(FinalPattern[ind][ind4]-SimGrid[menx-xx:menx-xx+ngsx,meny-yy:meny-yy+ngsy,trab]))
                           #p = p.sum()
                           p1 = Pond[menx-xx:menx-xx+ngsx,meny-yy:meny-yy+ngsy,trab]*(abs(FinalPattern[ind][ind4]-SimGrid[menx-xx:menx-xx+ngsx,meny-yy:meny-yy+ngsy,trab]))
                           p1 = p1.sum()
                           #promActual = SimGrid[menx-xx:menx-xx+ngsx,meny-yy:meny-yy+ngsy,trab]*AvLoc[menx-xx:menx-xx+ngsx,meny-yy:meny-yy+ngsy,trab]
                           #promActual=promActual.sum()
                           #p=p1*(1-w0)+w0*(abs(meanSG[menx][meny] - promActual)) 
                           promActual=np.mean(FinalPattern[ind][ind4])
                           p=p1*(1-w0)+w0*(abs(meanSG[menx][meny] - promActual))                        
   ...
   if nmgrid != 1 :
      for ind3 in xrange(nmgrid):
         ...
         dondey = -99.0*np.ones(t)
         #dondez = -99.0*np.ones(t)
         xx = (2**(ind))*(ngsx-1)/2 #es el espacio que utilizara la grilla de busqueda o filtro
         yy = (2**(ind))*(ngsy-1)/2
         #zz = (2**(ind))*(ngsz-1)/2 
         xx2 = (2**(ind))*(ngsx-innerx)/2 #Es el espacio que queda entre la grilla de busqueda y la grilla interna
         yy2 = (2**(ind))*(ngsy-innery)/2 
         #zz2 = (2**(ind))*(ngsz-innerz)/2   
         xx3 = (2**(ind))*(innerx-1)/2 #es el espacio que queda entre el centro y una orilla de la grilla interna
         ...
         #print GSdatos[ind]        
         # ACA DEBO AGREGAR GSdatos PARA QUE SE REINICIE EN 1
         #GSdatos[ind,xx2:sgnumx-(ngsx-innerx) +xx2,yy2:sgnumy-(ngsy-innery)+yy2] = 1
         #com = GSdatos[ind].sum() 
         ...
         while (com>0):
            ...
            for d in xrange(t): # Run all over each point
               ...
               if menx != -99 and meny != -99:
                  ...
                  if Listos[ind][trab][menx][meny] == 1: #cambie Listos por GSdatos
                  #if GSdatos[ind][menx][meny] == 1: #cambie Listos por GSdatos
                     
                     com1=GSdatos[ind,menx-xx3:menx+xx3,meny-yy3:meny+yy3].sum()
                     #print "com1 es:", com1
                     
                     '''
                     if ind==0: # aca se genera la dual template
                       #print "por aqui"
                  ...
                  if cntd>=minimumdc and vuelta == 1:
                     for ind4 in xrange(ncluster):
                        ...
                        if cont[ind][ind4]>0:
                           ...
                           #p1 = GSval[ind,menx-xx:menx+xx,meny-yy:meny+yy]*(Pond[menx-xx:menx+xx,meny-yy:meny+yy,trab]*(abs(FinalPattern[ind][ind4]-SimGrid[menx-xx:menx+xx,meny-yy:meny+yy,trab])))
                           p1 = p1.sum()
                     ...
                     for i in xrange(innerx):
                        for j in xrange(innery):
                           #for kk in xrange(innerz):
                              # This is the center of pattern respecto to the T.I.
                           #ttx = (2**(ind))*ii + menx - (2**(ind))*((innerx-1)/2)
                           #tty = (2**(ind))*jj + meny - (2**(ind))*((innery-1)/2)
                           #ttz = (2**(ind))*kk + menz - (2**(ind))*((innerz-1)/2)                   
                           ssx = i + (ngsx-1)/2 - ((innerx-1)/2)                       
                           ssy = j + (ngsy-1)/2 - ((innery-1)/2)
                           #ssz = kk + dk - ((innerz-1)/2)
                           #GSdatos[ind][ttx][tty] = 0
                           if NonMoveData[(2**(ind))*i + menx - (2**(ind))*((innerx-1)/2)][(2**(ind))*j + meny - (2**(ind))*((innery-1)/2)] != 1: # Here, each conditional data is not repleaced 
                           #if NonMoveData[ttx][tty] != 1: # Here, each conditional data is not repleaced 
                     ...
                     #Here, the locations of the values inside the inner grid are realesed             
                     #GSdatos[ind,menx - (2**(ind))*((innerx-1)/2):menx + (2**(ind))*((innerx-1)/2),meny - (2**(ind))*((innery-1)/2):meny + (2**(ind))*((innery-1)/2)] = 0                    
                     #GSdatos[ind][menx][meny]=0
                     #GSdatos[ind,menx-xx3:menx+xx3,meny-yy3:meny+yy3]=0
                     GSdatos[ind,menx-xx3:menx+xx3,meny-yy3:meny+yy3]=0
                     
                     #Listos[ind][trab][menx][meny] = 
                     Listos[ind][trab][menx][meny] = 0
                     #GSdatos[ind,menx-xx3:menx-xx3+innerx,meny-yy3:meny-yy3+innery] = 0
                     
                     #for i in xrange(innerx):
                        #for j in xrange(innery):
                           #for k in xrange(innerz):
                           #if NonMoveData[(2**(ind))*i + menx - (2**(ind))*((innerx-1)/2)][(2**(ind))*j + meny - (2**(ind))*((innery-1)/2)] != 1:
                              #Pond[(2**(ind))*i + menx - (2**(ind))*((innerx-1)/2)][(2**(ind))*j + meny - (2**(ind))*((innery-1)/2)][trab] = w2
                  
                     dondex[GSind[ind][menx][meny]] = -99.0
                     dondey[GSind[ind][menx][meny]] = -99.0
                     #dondez[GSind[ind][menx][meny][menz]] = -99.0  
                  if vuelta>1:
                     # verifica que no exista espacio vacio al interior
...
#with open('gt65.txt', 'wb') as csvfile:
with open('NUEVO0.0Imp_wo0.0_Nmgrid2.txt', 'wb') as csvfile:
...
#Now press enter to stop the currently process
#parasalirapretarenter = raw_input("Press enter to exit ")
